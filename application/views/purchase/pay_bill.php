...
<h4>Pay Bill Records</h4>

<div class="filterable">

    <div class="pull-right">
        <a href="<?= base_url("purchase/create/pay/bill") ?>" class="btn btn-info btn-xm"><span class="glyphicons glyphicons-plus"></span> Add New</a>
    ...
    <table id="datatable" class="table">
        ...
        <tbody>
            ...
            <?php foreach ($pay_bills as $key => $pay_bill): ?>

                <tr>
                    <td><?= ++$key ?></td>
                    <td><?= date("m/d/Y", strtotime($pay_bill->date)) ?></td>
                    <td><?= $pay_bill->supplier ?></td>
                    <td><?= $pay_bill->payment_type ?></td>
                    <td><?= $pay_bill->account ?></td>
                    <td><?= $pay_bill->amount ?></td>
                    <td>
                        <a href="<?= base_url("purchase/edit/pay/bill/$pay_bill->id") ?>" class="btn btn-success"><i class="fa fa-edit"></i></a>
                        <?php if (($this->session->userdata['logged_in']['role'] == 1) || ($this->session->userdata['logged_in']['role'] == 5)): ?>
                            <a href="<?= base_url("purchase/delete/pay/bill/$pay_bill->id") ?>" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                        <?php endif ?>
                    </td>
                </tr>
